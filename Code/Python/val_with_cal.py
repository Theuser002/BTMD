...
from Calibration.Cal_Model import CalNet
...
def val_with_cal (fold, val_loader, criterion, config):
    ...
    class_model = TestNet(class_in_features, config['n_classes'])
    cal_model = CalNet(cal_in_features, config['cal_n_classes'])
    
    CLASS_BEST_STATE_PATH = os.path.join(config['BEST_STATES_DIR'], f'{fold}_best_state.pth')
    CAL_BEST_STATE_PATH = os.path.join(config['CAL_BEST_STATES_DIR'], f'{fold}_best_cal_state.pth')
...
if __name__ == "__main__":
    ...
    for i in range(1, 6):
        ...
        BEST_STATE_PATH = os.path.join(cfg['BEST_STATES_DIR'], f'{outer_fold}_best_state.pth')
